...
for i in range(1, len(players)):
    ...
    doc.tables[1].cell(i, 1).text = str(players[i][1])
    doc.tables[1].cell(i, 2).text = players[i][0]
...
for i in range(1, len(teams)):
    ...
    doc.tables[0].cell(i, 0).text = str(i)
    doc.tables[0].cell(i, 1).text = teams[i][0]
    doc.tables[0].cell(i, 2).text = str(teams[i][1])
